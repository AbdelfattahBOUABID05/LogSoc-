...
export class ReportComponent implements OnInit, AfterViewInit {
  ...
  filterLogs(): void {
    ...
    if (this.errorLogs.length === 0 && this.warningLogs.length === 0 && this.infoLogs.length === 0 && this.analysis.segments) {
      const s = this.analysis.segments;
      // On accepte les clés en majuscules (venant du backend) ou minuscules
      const rawErrors = s.ERROR || s.error || s.critique || s.critical || s.errors || [];
      const rawWarnings = s.WARNING || s.warning || s.avertissement || s.warnings || [];
      const rawInfos = s.INFO || s.info || [];

      // Mapping en objets si nécessaire pour assurer la compatibilité avec le template
      this.errorLogs = Array.isArray(rawErrors) ? rawErrors.map(l => typeof l === 'string' ? { message: l, level: 'error' } : l) : [];
      this.warningLogs = Array.isArray(rawWarnings) ? rawWarnings.map(l => typeof l === 'string' ? { message: l, level: 'warning' } : l) : [];
      this.infoLogs = Array.isArray(rawInfos) ? rawInfos.map(l => typeof l === 'string' ? { message: l, level: 'info' } : l) : [];
    }
