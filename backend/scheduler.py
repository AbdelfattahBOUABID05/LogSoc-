...
def init_scheduler(app: Flask):
    """
    Initialise le planificateur APScheduler avec l'application Flask.
    Recharge automatiquement tous les jobs actifs depuis la base SQLite au démarrage.
    """
    if scheduler.running:
        logger.info("APScheduler est déjà en cours d'exécution.")
        return
    ...
    with app.app_context():
        ...
        # Récupération des tâches dont le statut est 'active'
        active_jobs = AnalysisJob.query.filter_by(status='active').all()
        for job in active_jobs:
            try:
                schedule_job(job)
            except Exception as e:
                logger.error(f"Erreur rechargement job {job.id}: {e}")
    
    logger.info("APScheduler initialisé et jobs actifs rechargés.")


def schedule_job(job):
    """
    Configure et ajoute une tâche répétitive dans le moteur APScheduler.
    Gère les fréquences : Horaire, Journalière, Hebdomadaire et Personnalisée.
    """
    job_id = f"analysis_job_{job.id}"
    
    # Définition du trigger en fonction de la fréquence choisie par l'utilisateur
    trigger_args = {}
    ...
    # Nettoyage si le job existe déjà pour éviter les doublons d'exécution
    if scheduler.get_job(job_id):
        scheduler.remove_job(job_id)

    # Ajout de la tâche au moteur de planification
    scheduler.add_job(
        id=job_id,
    ...
    logger.info(f"Job {job_id} planifié (Fréquence: {job.frequency})")


def run_planned_analysis(job_id: int):
    """
    Cœur de l'analyse automatisée. 
    Cette fonction est appelée périodiquement par le scheduler.
    1. Se connecte au serveur distant via SSH.
    2. Récupère les logs.
    3. Analyse les logs via le parser et l'IA Gemini.
    4. Enregistre les résultats dans l'historique SOC.
    """
    from extensions import db
    ...
    with flask_app.app_context():
        try:
            ...
            logger.info(f"[PLANIFICATEUR] Lancement de l'audit automatique pour {job.target_ip}")

            # Déchiffrement sécurisé du mot de passe SSH pour la connexion
            ...
            # Lecture des 500 dernières lignes du fichier log cible
            ...
            # Génération du rapport de sécurité par Intelligence Artificielle
            from utils import generate_security_summary
            ai_metrics = generate_security_summary(model=None, log_text=log_content)

            # Enregistrement du résultat comme une nouvelle analyse d'historique
            analysis = Analysis(
                user_id=job.user_id,
                job_id=job.id,
            ...
            # Mise à jour de la date de dernière exécution du job
            job.last_run_at = datetime.now(timezone.utc)
            db.session.commit()
            logger.info(f"[PLANIFICATEUR] Rapport d'audit {analysis.id} généré pour le job {job_id}")

            # Création d'une notification prioritaire si l'analyse est critique ou contient des menaces
            if ai_metrics.get('status') == 'Critique' or ai_metrics.get('menaces', 0) > 0:
                try:
                    status_label = ai_metrics.get('status', 'Inconnu')
                    menaces_count = ai_metrics.get('menaces', 0)
                    notification = Notification(
                        user_id=job.user_id,
                        title="Alerte de Sécurité Planifiée",
                        message=f"Niveau {status_label} détecté sur {job.target_ip} ({menaces_count} menaces)",
                        type="error" if ai_metrics.get('status') == 'Critique' else "warning",
...
def ssh_analyze_for_job(job: 'AnalysisJob') -> 'Analysis' | None:
    """
...
def check_anomalies(analysis: 'Analysis') -> bool:
    """
